...
private:
    MatrixType_t tMType;
    int16_t tXMult, tYMult;
    int16_t tWidth, tHeight;
    CRGB *p_LED;
    int16_t m_absWidth, m_absHeight;

public:
    cLEDMatrix(MatrixType_t tMType, int16_t tWidth, int16_t tHeight, int16_t tXMult = 0, int16_t tYMult = 0) {
        this->tMType = tMType;
        this->tXMult = tXMult;
        this->tYMult = tYMult;
        this->tWidth = tWidth;
        this->tHeight = tHeight;
        m_absWidth = (tWidth * ((tWidth < 0) * -1 + (tWidth > 0)));
        m_absHeight = (tHeight * ((tHeight < 0) * -1 + (tHeight > 0)));
//        p_LED = new CRGB[(m_absWidth * m_absHeight * ((tXMult == 0) && (tXMult == 0))) + ((tXMult != 0) && (tXMult != 0))]; // Will always be at least 1
...
//        }
    ...
    virtual uint16_t mXY(uint16_t x, uint16_t y) {
        ...
        if (tMType == HORIZONTAL_MATRIX) {
            if ((tXMult == 0) && (tYMult == 0)) return ((y * m_absWidth) + x);
            else return ((y * m_absWidth * tYMult) + (x * tXMult));
        } else if (tMType == VERTICAL_MATRIX) {
            if ((tXMult == 0) && (tYMult == 0)) return ((x * m_absHeight) + y);
            else return ((x * m_absHeight * tXMult) + (y * tYMult));
        } else if (tMType == HORIZONTAL_ZIGZAG_MATRIX) {
            if (y % 2) {
                if ((tXMult == 0) && (tYMult == 0)) return ((((y + 1) * m_absWidth) - 1) - x);
                else return ((((y + 1) * m_absWidth * tYMult) - tXMult) - (x * tXMult));
            } else {
                if ((tXMult == 0) && (tYMult == 0)) return ((y * m_absWidth) + x);
                else return ((y * m_absWidth * tYMult) + (x * tXMult));
            }
        ...
        {
            if (x % 2) {
                if ((tXMult == 0) && (tYMult == 0)) return ((((x + 1) * m_absHeight) - 1) - y);
                else return ((((x + 1) * m_absHeight * tXMult) - tYMult) - (y * tYMult));
            } else {
                if ((tXMult == 0) && (tYMult == 0)) return ((x * m_absHeight) + y);
                else return ((x * m_absHeight * tXMult) + (y * tYMult));
            }
